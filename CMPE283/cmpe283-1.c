...
struct capability_info processorbased[20] =
{
...
struct capability_info sec_processorbased[27] =
{
...
detect_vmx_features(void)
{
	...
	pr_info("Processor based Controls MSR: 0x%llx\n",	
		(uint64_t)(lo | (uint64_t)hi << 32));	
	report_capability(processorbased, 20, lo, hi);
	...
	pr_info("Secondary Processor based Controls MSR: 0x%llx\n",	
		(uint64_t)(lo | (uint64_t)hi << 32));	
	report_capability(sec_processorbased, 27, lo, hi);	
